...
if (isset($_POST['login_btn']))
{
  ...
  $validate = mysqli_query($conn,"SELECT * FROM user_login WHERE contact='$login_mobile_no' AND is_deleted='0' AND status='0'");
    ...
    if(mysqli_num_rows($validate)>0)
    {
      ...
      if ($db_pass==$login__pass_hash)
      {
        ...
        if ($data['status']==0)
        {
          ...
          redirect("../index.php?status=0");
        }else
        {
          // user is blocked so send a error msg
          redirect("../login.php?status=3");
        }

      }else
      {
        // password not match so send user a error msg..
        redirect("../login.php?status=2");
      }
    }else
    {
      // echo "user not found";
      redirect("../login.php?status=1");
    }

  }else
  {
    // echo "Server Not responding";
    redirect("../login.php?status=0");
